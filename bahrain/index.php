...
<body class="<?php echo $language == 'ar' ? 'arabic' : 'english'; ?>">
    ...
    <div class="image-container">
        ...
        <img src="orangelogo.svg" alt="Orange Logo" class="orange-logo">
        <img src="centerlogo.svg" alt="Logo" class="logo">
        <img src="centerlogoup.svg" alt="Logo Up" class="logo-up">
        <img src="audioon.svg" alt="Audio On" class="audioon">
    </div>
    ...
    <script>
        document.querySelector('.image-container img.logo').addEventListener('click', function() {
            var logo = this;
            if (logo.getAttribute('src') === 'centerlogo.svg') {
                logo.setAttribute('src', 'orangelogo.svg');
            } else {
                logo.setAttribute('src', 'centerlogo.svg');
            }
        });
        document.addEventListener('DOMContentLoaded', function() {
            var firstSvg = document.querySelector('.image-container img.logo');
            var secondSvg = document.querySelector('.image-container img.orange-logo');
            secondSvg.style.display = 'none';
            firstSvg.addEventListener('click', function() {
                secondSvg.style.display = secondSvg.style.display === 'none' ? 'block' : 'none';
            });
        });
        document.addEventListener('DOMContentLoaded', function() {
            var audioSvg = document.querySelector('.image-container img.audioon');
            audioSvg.addEventListener('click', function() {
                if (audioSvg.getAttribute('src') === 'audio.svg') {
                    audioSvg.setAttribute('src', 'audioon.svg');
                } else {
                    audioSvg.setAttribute('src', 'audio.svg');
                }
            });
        });
        function toggleMenu() {
